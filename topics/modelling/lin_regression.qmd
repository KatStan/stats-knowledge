...
## TL;DR
-  Do three diagnostics seen in <a href="#" onclick="goToTab('tabset-1-3-tab'); return false;">Third Tab</a>
-  Check manual and adjusted $R^2>>0$
-  Check residual standard error is small for the units and mean of your response variable 
-  Continue looking at traits by following in <a href="#" onclick="goToTab('tabset-1-5-tab'); return false;">Fifth Tab</a>  
...
-  Additive function f is such that this holds $$ f_{v_1+v_2}(x) = f_{v_1}(x) + f_{v_2}(x), \text{ and} \\ f_{c*v_1}(x) = c*f_{v_1}(x) $$ where $$ v_1 = (\beta_0, \beta_1, ..., \beta_k), ~ v_2 = (\alpha_0, \alpha_1, ..., \alpha_k)$$ i.e. the coefficients of $f(x)$. See the maths behind <a href="#" onclick="goToTab('tabset-1-9-tab'); return false;">Last Tab</a> for proof this holds for $E(Y|x) = m(x) = \beta_0 + \sum_{j=1}^k \beta_jg_{j}(x)$
...
Apart from doing everything in <a href="#" onclick="goToTab('tabset-1-3-tab'); return false;">Third tab</a>, especially 1), there is also $R^2$

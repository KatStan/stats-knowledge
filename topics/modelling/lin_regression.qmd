...
-  The residuals summary has a median close to zero (in units of response var) 
...
-  Additive function f is such that this holds $$ f_{v_1+v_2}(x) = f_{v_1}(x) + f_{v_2}(x), \text{ and} \\ f_{c*v_1}(x) = c*f_{v_1}(x) $$ where $$ v_1 = (\beta_0, \beta_1, ..., \beta_k), ~ v_2 = (\alpha_0, \alpha_1, ..., \alpha_k)$$ i.e. the coefficients of $f(x)$. See the maths behind <a href="#" onclick="goToTab('tabset-1-9-tab'); return false;">Maths</a> for proof this holds for $E(Y|x) = m(x) = \beta_0 + \sum_{j=1}^k \beta_jg_{j}(x)$
...
## Reporting   
Depends on what type your data is and whether it was scaled 
...
3. [https://numiqo.com/tutorial/linear-regression](https://numiqo.com/tutorial/linear-regression)
4. [https://www.statology.org/interpret-regression-output-in-r/](https://www.statology.org/interpret-regression-output-in-r/)
5. [https://feliperego.github.io/blog/2015/10/23/Interpreting-Model-Output-In-R](https://feliperego.github.io/blog/2015/10/23/Interpreting-Model-Output-In-R)
6. [https://r-statistics.co/Interpreting-Regression-Output-Completely.html](https://r-statistics.co/Interpreting-Regression-Output-Completely.html)
:::